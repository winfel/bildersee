 ...
 */
 ...
 include_once('../server/environment.php');
...
session_start();
    ...
    @$uploadermail=$album['uploadermail'];
...
$tfu_config_version = '2.14';
    ...
    $folder = $folder = '../upload_pics/'.$albumname;                  // this is the root upload folder. If you use login='auth' by default the folder from the user profile in .htusers.php is used!
    ...
    $resize_data = '100000'; // The data for the resize dropdown
    $resize_label = 'Original'; // The labels for the resize dropdown
    ...
    $show_delete = 'false';               // Shows the delete button - if download is set to button this moves to the menu!
    ...
    $upload_notification_email = 'winkelnkemper@googlemail.com';     // you can get an email everytime a fileupload was initiated! The mail is sent at the first file of an upload queue! '' = no emails - php mail has to be configured properly! Enter the e-mail you want to send the notification to.
    $upload_notification_email_from = 'upload@bildersee.eu'; // the sender of the notification email!
    $upload_notification_email_subject = 'Public Upload'; // Subject of the email - you should set a nicer one after the login or in tfu_upload.php
    

    
    
    $upload_notification_email_text = "Öffentlicher Upload ($albumname) von $uploader ($uploadermail %s): %s \n\n$remarks"; // Text of the email - the first %s ist the username (if no is set 'not set is used'), the 2nd %s is the list of files that where uploaded!
    ...
    $exclude_directories = array('data.pxp', '_vti_cnf', 'CVS', 'thumbs', '.svn'); // new 2.6 - You can enter directories here that are ignored in TFU. You can enter files as well that should be hidden!
    ...
    $normalise_file_names = true;         // new 2.7.5 - This setting convertes all filenames to lowercase and special characters are removed e.g. !"#$%&'()*+,-- []\^_`öäüß are replaces with an _
    $normalise_directory_names = true;   // new 2.8.1 - This setting convertes all directory names that are created or renamed to lowercase and special characters are removed e.g. !"#$%&'()*+,-- []\^_`öäüß are replaces with an _
     ...
     */
    $titel = 'Bildersee Uploader';       // This is the title of the flash - can not be set in the freeware version!
    ...
    $enable_file_download = 'false';   // You can enable the download of files! valid entries 'true', false', 'button', 'button1' - 'button' show the dl button insted the menu button if all other elements of the menu are set to false - 'button1' shows the download button instead of the delete button and the delete function is moved to the menu if enabled! - only available in the registered version!
      $download_multiple_files_as_zip = 'false'; // You can enable that multiple files are downloadd as one zip. The zip files are created dynamically. Therefore it could take some time until the download starts.
    ...
    $enable_folder_move = 'false';        // Show the menu item to move folders - you need a professional license or above to use this
    $enable_file_copymove = 'false';      // Show the menu item to move and copy files - you need a professional license or above to use this
    ...
    $forbidden_view_file_extensions = 'php,html,htm'; // If you have set $allowed_view_file_extensions = 'all' then you can define a list of file extensions that are not shown. If you set a extra , at the end files with no extension are not viewed as well. - Only available for registered users.
