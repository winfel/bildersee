...
<html>
 <head>
  ...
  <style>
  	...
  	#id {display:none;position:absolute;top:0;padding-top:20%;left:0;width:100%;height:100%;background-color:black;font-size:60px;text-align:center}
...
function getPresenterData(){
	if (getterLocked) return;
	getterLocked=true;
	var id='<?php echo $id ?>';
	server_query("getPresenterData.php?id="+id,function(value){
		getterLocked=false;
...
function processData(data){
	if (data=='showid') {
		...
		div.innerHTML='<img src="<?php echo $config->imageGetterURL; ?>?key='+data+'&width=1000000&height=1000" onload="show('+num+');">';
	}
}
