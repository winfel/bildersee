<?php
	    ...
	    if (!$pageTitle || $pageTitle=='%' || $pageTitle=='%%') {
	    	...
	    	$pageTitle=trim(substr($pageTitle,strpos($pageTitle,' ')));
	    }
		...
		if (!$folderGiven && $tagGiven) {
			$activePart='tags';
			$element=array();$element['link']='?mode=tags';$element['text']='Tags';$breadcrumb[]=$element;
		} else {
		...
		if ($folderGiven && $tagGiven){
			...
			if ($folderGiven){
				$element=array();$element['link']='';$element['text']=$pageTitle;$breadcrumb[]=$element;
			}
		}
		...
		if ($tagGiven) {
			...
			$temp=str_replace('notag_','nicht ',$temp);
		...
		$search=mysql_query("SELECT `key`,files.tags as tags, filetags.tags as filetags,subfolder,copyright,folder,sortstring  FROM files LEFT JOIN filetags ON files.`key`=filetags.`image` WHERE $userQuery AND files.folder LIKE '$folder' $filterSQL $order $reverse");
		...
		while ($line=mysql_fetch_object($search)){
			...
			if ($codewordPossible && !$codeword){
				$codeword=explode('codeword_',$line->tags);
				$codeword=$codeword[1];
				$codeword=explode(' ',$codeword);
				$codeword=$codeword[0];
			}
			$copyrights[ucwords_new(str_replace('_',' ',$line->copyright))]=true;
		...
		if (true){
			...
			echo '<p>'.translate('Your search did not return any results!').'</p>';
		}
